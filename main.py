...
async def send_initial_state(parsed: Dict[str, dict]):
    ...
    await send_message("🚀 <b>Monitoring Started</b>")

    down_grouped = group_by_rrset(parsed, value=0)
    up_grouped = group_by_rrset(parsed, value=1)

    await send_grouped_messages(down_grouped, "🟥 <b>DOWN ({total}):</b>")
    await send_grouped_messages(up_grouped, "🟩 <b>UP ({total}):</b>")
...
async def send_down_reminder():
    ...
    lines = [f"⏰ <b>DOWN Reminder ({total}):</b>"]
